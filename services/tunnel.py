...
async def _update_menu_button(bot, twa_url: str):
    ...
    try:
        ...
        logger.info(f"✅ Меню-кнопку бота успішно оновлено на URL: {twa_url}")

    except Exception as e:

        logger.error(f"Не вдалося оновити меню-кнопку бота: {e}")





async def start_tunnel(bot, port: int = None) -> str | None:
    ...
    if current_twa and "localhost" not in current_twa and "127.0.0.1" not in current_twa:
        ...
        if await _check_url_accessible(current_twa):
            ...
            await _update_menu_button(bot, current_twa)
    ...
    url = await _start_localhost_run(bot, port)
    ...
    if url:
        ...
        await _update_menu_button(bot, twa_url)
...
async def _start_localhost_run(bot, port: int) -> str | None:
    ...
    try:
        ...
        if url:
            ...
            asyncio.create_task(_monitor_tunnel_process(_tunnel_process, bot, port))
...
async def _monitor_tunnel_process(process: asyncio.subprocess.Process, bot, port: int):
    ...
    while True:
        ...
        try:
            ...
            new_url = await _start_localhost_run(bot, port)
            ...
            if new_url:
                ...
                await _update_menu_button(bot, config.TWA_URL)
