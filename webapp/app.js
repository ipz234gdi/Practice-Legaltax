...
async function loadRequests() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/twa/my-requests?user_id=${userId}`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
async function loadAdminPending() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/twa/admin/pending?admin_id=${userId}`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
async function handleAdminAction(requestId, action) {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/twa/admin/action`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        admin_id: userId,
