...
async def send_next_queue_card(bot, chat_id, state: FSMContext):
    ...
    async with SessionLocal() as session:
        # Шукаємо першу pending заявку, яку адмін ще не бачив у цій сесії
...
@router.message(Command("pending"), is_admin_filter)
...
@router.message(F.text == "Панель адміністратора ⚙️", is_admin_filter)
async def show_admin_panel(message: Message):
    async with SessionLocal() as session:
        ...
        pending_forms_query = await session.execute(
            select(func.count(RequestForm.id)).where(RequestForm.status == "pending")
        )
        pending_forms = pending_forms_query.scalar() or 0

        in_progress_forms_query = await session.execute(
            select(func.count(RequestForm.id)).where(RequestForm.status == "in_progress")
        )
        in_progress_forms = in_progress_forms_query.scalar() or 0

        completed_forms_query = await session.execute(
            select(func.count(RequestForm.id)).where(RequestForm.status == "completed")
        )
        completed_forms = completed_forms_query.scalar() or 0
    ...
    admin_info = (
        ...
        f"⏳ Очікують: `{pending_forms}`\n"
        f"⚙️ В роботі: `{in_progress_forms}`\n"
        f"✅ Виконано/Відповіли: `{completed_forms}`\n"
        f"───────────────────\n"
...
async def process_admin_work(callback: CallbackQuery):
    ...
    # Залишаємо кнопку "Відповісти" та кнопку "Вийти з черги"
    builder = InlineKeyboardBuilder()
...
async def process_admin_reject_confirm(callback: CallbackQuery, state: FSMContext):
    req_id = int(callback.data.split(":")[1])
...
async def process_admin_pending(callback: CallbackQuery, state: FSMContext):
    ...
    # Якщо адмін у черзі, показуємо наступну картку
    state_status = await state.get_state()
    if state_status == AdminStates.in_queue.state:
        await send_next_queue_card(callback.bot, callback.message.chat.id, state)
...
async def process_admin_reply_start(callback: CallbackQuery, state: FSMContext):
    ...
    # Зберігаємо дані стану черги, щоб повернутися після відповіді
    ...
    await safe_send_or_edit(
        ...
        text=f"✍️ *Введіть повідомлення для відповіді клієнту* \\(ID заявки: \\#{req_id}, Клієнт: {escape_markdown(req.name)}\\):\n"
             f"_\\(Для скасування натисиниіть кнопку нижче\\)_",
        parse_mode="MarkdownV2",
...
async def process_admin_reply_send(message: Message, state: FSMContext):
    ...
    if success:
        ...
        # Якщо не вдалося надіслати в ТГ (наприклад, заблокував бота або заявка прийшла з сайту/пошти)
        await safe_send_or_edit(
            message.answer,
    ...
    if was_in_queue:
        ...
        # Відновлюємо головне меню для адміна
        from handlers.common import get_main_menu_keyboard
        await safe_send_or_edit(
            message.answer,
